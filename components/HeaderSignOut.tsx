...
export default function HeaderSignOut() {
  ...
  const doPrimary = async () => {
    ...
    try {
      ...
      Alert.alert(mode === 'signin' ? 'Sign in failed' : 'Sign up failed', e?.message || 'Try again');
    } finally {
      setBusy(false);
    }
  };
  ...
  if (!signedIn) {
    // Signed-out: show Sign in button + modal with Sign in / Sign up
    return (
      <View style={{ marginRight: 12 }}>
        <Pressable
          onPress={() => { setMode('signin'); setOpen(true); }}
          hitSlop={8}
          style={({ pressed }) => ({
            paddingVertical: 6,
        ...
        <Modal visible={open} transparent animationType="fade" onRequestClose={() => !busy && setOpen(false)}>
          ...
          >
            <View style={{ backgroundColor:'#0f0f10', borderRadius:12, borderColor:BORDER, borderWidth:1, padding:16 }}>
              <Text style={{ color:'#fff', fontWeight:'900', fontSize:16, marginBottom:8 }}>
              ...
              <View style={{ flexDirection:'row', justifyContent:'space-between', alignItems:'center', marginBottom:10 }}>
                <Pressable onPress={() => setMode(mode === 'signin' ? 'signup' : 'signin')} disabled={busy} hitSlop={6}>
                  <Text style={{ color: ORANGE, textDecorationLine:'underline', fontWeight:'800' }}>
              ...
              <View style={{ flexDirection:'row', justifyContent:'flex-end', gap:8 }}>
                <Pressable
                  ...
                  disabled={busy}
                  style={{ paddingVertical:8, paddingHorizontal:12, borderRadius:8, borderWidth:1, borderColor:BORDER }}
                >
                ...
                <Pressable
                  onPress={doPrimary}
                  disabled={busy}
                  style={{ paddingVertical:8, paddingHorizontal:12, borderRadius:8, borderWidth:1, borderColor:ORANGE }}
                >
                  {busy
                    ? <ActivityIndicator color={ORANGE} />
                    : <Text style={{ color: ORANGE, fontWeight:'800' }}>{mode === 'signin' ? 'Sign in' : 'Create'}</Text>
                  }
                </Pressable>
              </View>
            </View>
