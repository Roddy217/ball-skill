...
import { getBalance, getUserJoins, grantCredits, unrecordJoin } from '../services/api';
import IdChip from '../components/IdChip';

// --- Local catalog to hydrate IDs (mirrors Events screen) ---
type CatalogEvent = {
  id: string;
  title: string;
  date: string;
...
export default function ProfileScreen() {
  ...
  const hydrate = useCallback((ids: string[]): CatalogEvent[] => {
    return ids
      ...
      .filter(Boolean) as CatalogEvent[];
  }, []);
  ...
  const onUnjoin = useCallback(async (ev: CatalogEvent) => {
    if (!hasEmail) return;
    Alert.alert(
      'Unjoin event',
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              // Refund (cents) on client; then remove the join server-side
              await grantCredits(email, ev.fee * 100, `unjoin:${ev.id}`);
              await unrecordJoin(ev.id, email);
              // Update local lists + balance
              setJoinedIds(prev => prev.filter(id => id !== ev.id));
              const b = await getBalance(email).catch(() => null);
              setBalance(b);
...
const s = StyleSheet.create({
  ...
  unBtnText: { color: colors.WHITE, fontWeight: '800' },
});
